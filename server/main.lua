...
local function registerActivity()
  ...
  local ok, result = pcall(function()
    return exports['Az-Summer2Core']:RegisterServerActivity(SummerActivity)
  end)

  if ok and result then
    registered = true
    print(('[%s] Registered Summer 2.0 server activity: %s'):format(GetCurrentResourceName(), SummerActivity.label or SummerActivity.id))
    return true
  end

  print(('[%s] Failed to register Summer 2.0 server activity.'):format(GetCurrentResourceName()))
  return false
end
